...
let advTransferEtherTo = $("#advTransferEtherTo");
let advTransferEtherValue = $("#advTransferEtherValue");
let advTransferEtherButton = $("#advTransferEtherButton");
  ...
  if (bankAddress != "") {
    ...
    $.get(
      ...
      function(result) {
        ...
        $("#coinBalance").text("NCCU Coin餘額 (wei): ");
      }
    );
  }
});
...
mintButton.on("click", async function() {
  ...
  $.post(
    ...
    function(result) {
      if (result.events !== undefined) {
        log(result.events.MintEvent.returnValues, "mint 成功");
...
buyCoinButton.on("click", async function() {
  ...
  $.post(
    ...
    function(result) {
      if (result.events !== undefined) {
        log(result.events.BuyCoinEvent.returnValues, "buy coin成功");
...
transferCoinButton.on("click", async function() {
  ...
  $.post(
    ...
    function(result) {
      if (result.events !== undefined) {
        log(result.events.TransferCoinEvent.returnValues, "transfer coin成功");
...
transferOwnerButton.on("click", async function() {
  ...
  $.post(
    ...
    function(result) {
      if (result.events !== undefined) {
        log(
          result.events.TransferOwnerEvent.returnValues,
          "transfer owner成功"
        );
...
advTransferEtherButton.on("click", async function() {
  if (bankAddress == "") {
    return;
  }
  ...
  $.post(
    "/advTransferTo",
    {
      address: bankAddress,
      account: nowAccount,
      to: advTransferEtherTo.val(),
      value: parseInt(advTransferEtherValue.val(), 10)
    },
    function(result) {
      if (result.events !== undefined) {
        log(result.events.TransferEvent.returnValues, "adv 轉帳成功");
